...
//    private var numItemsThreshold = 30
    ...
    private fun postProcessOBB(
        ...
        // Auto-detect coordinate format: pixel (0..inputSize) vs normalized (0..1)
        // iOS always divides by inputW/inputH. TFLite may or may not need it.
        // Check first few high-confidence detections to determine format.
        val (inputW, inputH) = modelInputSize
        val needsNormalization = detectCoordinateFormat(detections2D, numClasses, confidenceThreshold, inputW, inputH)
        val scaleX = if (needsNormalization) inputW.toFloat() else 1f
        val scaleY = if (needsNormalization) inputH.toFloat() else 1f

        if (needsNormalization) {
            Log.d("ObbDetector", "OBB output is in PIXEL space → normalizing by ${inputW}x${inputH}")
        } else {
            Log.d("ObbDetector", "OBB output is already NORMALIZED (0..1)")
        }
        ...
        for (i in 0 until anchorsCount) {
            ...
            // Normalize to [0..1] like iOS does (divide by model input size)
    ...
    /**
     * Auto-detect if model outputs pixel coordinates (0..inputSize) or normalized (0..1).
     * Scans the top confident detections: if any cx/cy value > 2.0, it's pixel space.
     * This handles both TFLite models that include normalization and those that don't.
     */
    private fun detectCoordinateFormat(
        detections2D: Array<FloatArray>,
        numClasses: Int,
        confidenceThreshold: Float,
        inputW: Int,
        ...
        for (i in detections2D.indices) {
            if (checked >= 50) break // check first 50 above-threshold detections
            val data = detections2D[i]
            ...
            if (bestScore < 0.1f) continue // skip very low confidence
            checked++
            val cx = data[0]
            val cy = data[1]
            val w = data[2]
        ...
        // If any coordinate > 2.0, values are in pixel space and need normalization
        val isPixelSpace = maxCoord > 2.0f
        Log.d("ObbDetector", "Coordinate format detection: maxCoord=$maxCoord, isPixelSpace=$isPixelSpace, modelInput=${inputW}x${inputH}")
        return isPixelSpace
    }
